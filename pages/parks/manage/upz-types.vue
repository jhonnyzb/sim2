...
export default {
  ...
  nuxtI18n: {
    paths: {
      en: '/parks/manage/upz-types',
      es: '/parques/administrar/tipos-de-upz',
    },
  },
  components: {
  ...
  meta: {
    ...
    permissions(bouncer, to, from) {
      ...
      const abilities = service.manageAbilities(service.models.UPZ_TYPE)
  ...
  computed: {
    title() {
      return this.$t('parks.park.upz_type')
    },
    canCreateAction() {
      return this.canManageOrCreate(this.ability_service.models.UPZ_TYPE)
    },
    canUpdateAction() {
      return this.canManageOrUpdate(this.ability_service.models.UPZ_TYPE)
    },
    canDeleteAction() {
      return this.canManageOrDestroy(this.ability_service.models.UPZ_TYPE)
    },
    canViewHiAction() {
      return this.canViewHistory(this.ability_service.models.UPZ_TYPE)
    },
  },
  methods: {
    getData() {
    ...
    onCreate(ref) {
      this.form = new UpzType()
      this.form.setFormInstance(ref)
    },
    onUpdate(item, ref) {
      this.form = new UpzType(item)
      this.form.setFormInstance(ref)
    },
    onDelete(item) {
      if (item.id) {
