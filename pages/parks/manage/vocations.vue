...
export default {
  ...
  nuxtI18n: {
    paths: {
      en: '/parks/manage/vocations',
      es: '/parques/administrar/vocaciones',
    },
  },
  components: {
  ...
  meta: {
    ...
    permissions(bouncer, to, from) {
      ...
      const abilities = service.manageAbilities(service.models.VOCATION)
  ...
  computed: {
    title() {
      return this.$t('parks.park.vocation')
    },
    canCreateAction() {
      return this.canManageOrCreate(this.ability_service.models.VOCATION)
    },
    canUpdateAction() {
      return this.canManageOrUpdate(this.ability_service.models.VOCATION)
    },
    canDeleteAction() {
      return this.canManageOrDestroy(this.ability_service.models.VOCATION)
    },
    canViewHiAction() {
      return this.canViewHistory(this.ability_service.models.VOCATION)
    },
  },
  methods: {
    subtextFormat(str) {
    ...
    onCreate(ref) {
      this.form = new Vocation()
      this.form.setFormInstance(ref)
    },
    onUpdate(item, ref) {
      this.form = new Vocation(item)
      this.form.setFormInstance(ref)
    },
    onDelete(item) {
      if (item.id) {
